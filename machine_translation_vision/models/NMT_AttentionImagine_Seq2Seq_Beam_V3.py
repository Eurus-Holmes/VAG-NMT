...
class NMT_AttentionImagine_Seq2Seq_Beam_V3(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 im_feats_size, \
        ...
        super(NMT_AttentionImagine_Seq2Seq_Beam_V3,self).__init__()
        ...
        self.imagine_attn = ImagineAttn(attn_model,self.hidden_size,self.shared_embedding_size)
    ...
    def forward(self,src_var,src_lengths,tgt_var,im_var,teacher_force_ratio=1.0,max_length=80, criterion_mt=None, criterion_vse=None):
        ...
        attn_weights = self.imagine_attn(im_embedding,decoder_hiddens)
        #print(attn_weights)
        ...
        text_embedding = self.text_embedding(context_vec)
        if self.activation_vse:
            text_embedding = F.tanh(text_embedding)

        if self.dropout_txt_emb > 0:
            text_embedding = self.txt_embedding_dropout(text_embedding)
