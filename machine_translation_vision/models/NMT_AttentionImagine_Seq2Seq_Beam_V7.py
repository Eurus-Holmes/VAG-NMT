#Implement an enhanced version of V6, where the Image Attn weighted sum of encoder hidden states will be used to initialize
#the hidden states of the decoder & Only h1 will be used to learn the shared space. This all happens during training. 
import torch
...
from ..layers import NMT_Decoder_V3
from ..layers import VSE_Imagine
from ..layers import VSE_Imagine_Enc_Dec_V2
...
class NMT_AttentionImagine_Seq2Seq_Beam_V7(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 im_feats_size, \
                 ...
                 activation_vse = True, \
                 tied_emb=False, \
                 im_retrieval_text_emb='dec', \
                 init_split=0.5):

        super(NMT_AttentionImagine_Seq2Seq_Beam_V7,self).__init__()
        ...
        self.im_retrieval_text_emb = im_retrieval_text_emb
        ...
        self.decoder = NMT_Decoder_V3(tgt_size,tgt_embedding_size,hidden_size,2*hidden_size,n_layers,dropout_rnn=dropout_rnn_dec,dropout_out=dropout_out,dropout_emb=0.0,tied_emb=tied_emb)
        
        #Initialize the VSE_Imagine Module
        self.vse_imagine = VSE_Imagine_Enc_Dec_V2(self.attn_model,self.im_feats_size, self.hidden_size, self.shared_embedding_size, self.dropout_im_emb, self.dropout_txt_emb, self.activation_vse)
        
        #self.vse_imagine_dec = VSE_Imagine(self.attn_model,self.im_feats_size, self.hidden_size, self.shared_embedding_size, self.dropout_im_emb, self.dropout_txt_emb, self.activation_vse)
        #Initialize the VSE_Imagine Module to embed the encoder hidden states to the shared space. 
        #self.vse_imagine_enc = VSE_Imagine(self.attn_model,self.im_feats_size,2*hidden_size,self.shared_embedding_size,self.dropout_im_emb,self.dropout_txt_emb,self.activation_vse)
        
        #Tie the im_embedding layers of the two Imagine Module
        #self.vse_imagine_dec.im_embedding.weight = self.vse_imagine_enc.im_embedding.weight
        #self.vse_imagine_dec.im_embedding.bias = self.vse_imagine_enc.im_embedding.bias

        #A potential third layer is built if we didn't observe a good shared space learning with the previous two layer.
        #Decoder Initialization Layer
        self.decoderini = nn.Linear(2*hidden_size,hidden_size)

        #Initilaize the layers with kaiming method
    ...
    def forward(self,src_var,src_lengths,tgt_var,im_var,teacher_force_ratio=1.0,max_length=80, criterion_mt=None, criterion_vse=None):
        ...
        #Compute the Weighted Sum of Encoder Outputs from Attn Image
        encoder_concat = self.vse_imagine.get_enc_context(im_var,encoder_outputs,ctx_mask=context_mask)

        #self.vse_imagine.get_enc_context
        ...
        #decoder_hidden = F.tanh(self.decoderini(encoder_concat)).unsqueeze(0)#Initialize the Decoder with Imagine Attention Weighted Sum
        ...
        if is_teacher: 
            for di in range(tgt_l):
                decoder_output,decoder_hidden,decoder_gru_1 = self.decoder(decoder_input, decoder_hidden, encoder_outputs, ctx_mask=context_mask)
                loss_n = criterion_mt(decoder_output,tgt_var[:,di])
                loss_mt += loss_n

                decoder_hiddens[di] = decoder_gru_1
            ...
            for di in range(tgt_l):
                ...
                #Update decoder hiddens with gru hidden state 1
                decoder_hiddens[di] = decoder_gru_1
                #text_embedding_sets[di] = text_embedding_di
        ...
        #Verify that vse_imagine_enc and vse_imagine_dec share the same im_embedding
        #assert torch.equal(self.vse_imagine_dec.im_embedding.weight.data,self.vse_imagine_enc.im_embedding.weight.data), "The im embedding layer between the two VSE_Imaine module is not tied"
        
        """
        #Compute the VSE loss with VSE_Imagine Module and deoder hidden states
        loss_vse_dec = self.vse_imagine_dec(im_var,decoder_hiddens,criterion_vse=criterion_vse)

        #Compute the VSE loss with VSE_Imagine Module and encoder hidden states
        loss_vse_enc = self.vse_imagine_enc(im_var,encoder_outputs,criterion_vse=criterion_vse,context_mask=context_mask)
        """
        loss_vse = self.vse_imagine(im_var,encoder_outputs,decoder_hiddens,criterion_vse=criterion_vse,context_mask=context_mask)
    ...
    def beamsearch_decode(self,src_var,src_lengths,im_var,beam_size=1,max_length=80,tgt_var=None):
        ...
        decoder_input = Variable(torch.LongTensor([[SOS_token] for x in range(batch_size)]))
        #decoder_hidden = F.tanh(self.decoderini(encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1))).unsqueeze(0)
        #decoder_hidden = F.tanh(self.decoderini(encoder_concat)).unsqueeze(0)#Initialize the decoder with attention weighted encoder hidden states
        decoder_hidden = F.tanh(self.decoderini(self.init_split*encoder_concat+(1-self.init_split)*encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1))).unsqueeze(0)
    ...
    def embed_sent_im_eval(self,src_var,src_lengths,tgt_var,im_feats):
        ...
        encoder_concat = self.vse_imagine.get_enc_context(im_feats,encoder_outputs,ctx_mask=context_mask)
        ...
        #Determine whether teacher forcing is used. 
        for di in range(tgt_l):
            decoder_output,decoder_hidden,decoder_hidden_gru_1 = self.decoder(decoder_input, decoder_hidden, encoder_outputs)
            #update text_embedding_sets
            decoder_hiddens[di] = decoder_hidden_1
            
            decoder_input = tgt_var[:,di]

        if self.im_retrieval_text_emb == 'dec':
            im_embedding,text_embedding = self.vse_imagine.get_emb_vec(im_feats,encoder_outputs,decoder_hiddens)
        else:
            im_embedding,text_embedding = self.vse_imagine.get_emb_vec(im_feats,encoder_outputs,decoder_hiddens,ctx_mask=context_mask)
    ...
    def embed_sent_im_test(self,src_var,src_lengths,im_feats,max_length=80):
        ...
        #Get the Weighted Sum Encoder Outputs
        encoder_concat = self.vse_imagine.get_enc_context(im_feats,encoder_outputs,ctx_mask=context_mask)
        ...
        for di in range(tgt_l):
            decoder_output,decoder_hidden,decoder_hidden_gru_1 = self.decoder(decoder_input, decoder_hidden, encoder_outputs)
            decoder_hiddens[di] = decoder_hidden_gru_1
            #text_embedding_sets[di] = text_embedding_di
            _,top1 = decoder_output.data.topk(1)
            decoder_input = Variable(top1)
            if use_cuda:
    ...
    def get_imagine_attention_eval(self,src_var,src_lengths,tgt_var,im_feats):
        ...
        decoder_translation_list = []
        for di in range(tgt_l):
            decoder_output,decoder_hidden,decode_hidden_gru_1 = self.decoder(decoder_input, decoder_hidden, encoder_outputs)
            #update text_embedding_sets
            decoder_hiddens[di] = decoder_hidden_gru_1
        ...
        decoder_vse_attn_weights = self.vse_imagine.get_imagine_weights(im_feats,encoder_outputs,decoder_hiddens)
        ...
        return decoder_vse_attn_weights.data,final_translations
    ...
    def get_imagine_attention_test(self,src_var,src_lengths,im_feats,max_length=80):
        ...
        #Get the attention weights
        if self.im_retrieval_text_emb == 'dec':
            vse_attn_weights = self.vse_imagine.get_imagine_weights(im_feats,encoder_outputs,decoder_hiddens)
        else:
            vse_attn_weights = self.vse_imagine.get_imagine_weights(im_feats,encoder_outputs,decoder_hiddens,ctx_mask=context_mask)
        #Get the transition_list
        final_translations = []
        for b in range(batch_size):
            current_list = []
