#Optimized V11, with sepearate decoder intiailized layer from the two separte hidden states. 
import torch
...
class NMT_AttentionImagine_Seq2Seq_Beam_V12(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 im_feats_size, \
        ...
        super(NMT_AttentionImagine_Seq2Seq_Beam_V12,self).__init__()
        ...
        #Decoder Initialization Layer Visual
        self.decoderini_v = nn.Linear(2*hidden_size,hidden_size,bias=False)
    ...
    def forward(self,src_var,src_lengths,tgt_var,im_var,teacher_force_ratio=1.0,max_length=80, criterion_mt=None, criterion_vse=None):
        ...
        decoder_hidden = F.tanh(self.decoderini_v(encoder_concat)+self.decoderini(encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1))).unsqueeze(0) # New way to initilaize
