#Implements a different VSE loss from https://github.com/linxd5/VSE_Pytorch
import torch
...
class NMT_AttentionImagine_Seq2Seq_Beam_V2(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 im_feats_size, \
        ...
        super(NMT_AttentionImagine_Seq2Seq_Beam_V2,self).__init__()
    ...
    def forward(self,src_var,src_lengths,tgt_var,im_var,teacher_force_ratio=1.0,max_length=80, criterion_mt=None, criterion_vse=None):
        ...
        if is_teacher: 
            for di in range(tgt_l):
                ...
                text_embedding_di = self.text_embedding(decoder_hidden.squeeze(0))
                if self.activation_vse:
                    text_embedding_di = F.tanh(text_embedding_di)

                if self.dropout_txt_emb > 0:
                    text_embedding_di = self.txt_embedding_dropout(text_embedding_di)

                #Normalize The Text Embedding Vector
                text_embedding_sets[di] = l2norm(text_embedding_di)
        ...
        im_embedding = self.im_embedding(im_var)
        if self.activation_vse:
            im_embedding = F.tanh(im_embedding)

        if self.dropout_im_emb > 0:
            im_embedding = self.im_embedding_dropout(im_embedding)
        #Normalize the image embedding vector
        im_embedding = l2norm(im_embedding) #im_embedding: B*(2*hidden_size)
        ...
        #Apply l2norm here as well
        text_embedding = l2norm(text_embedding)

        #Apply l2norm here to conver im_embedding to normalized vector
        #im_embedding = l2norm(im_embedding)

        loss_vse = criterion_vse(im_embedding,text_embedding)
    ...
    def embed_im(self,im_feats):
        """
            Embed the CNN features to the shared space
            Input:
                im_feats: The Image Features with size (B,D), D is the dimension of image feature.
            Output:
                im_embedded_features: The embedded image vectors with size (B,SD), SD is the dimension of the 
                shared embedding space.
        """
        im_embedding = self.im_embedding(im_feats)
        im_embedding = l2norm(im_embedding)
    ...
    def embed_sent_im_eval(self,src_var,src_lengths,tgt_var,im_feats):
        ...
        #Compute the text imbedding based on attention
        attn_weights = self.imagine_attn(im_embedding,text_embedding_sets)
    ...
    def embed_sent_im_test(self,src_var,src_lengths,im_feats,max_length=80):
        ...
        #Normalize the im_embedding here.
        #im_embedding = l2norm(im_embedding)
