...
from ..layers import LIUMCVC_Decoder
...
class LIUMCVC_Seq2Seq_Beam(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 src_embedding_size, \
        ...
        super(LIUMCVC_Seq2Seq_Beam,self).__init__()
        ...
        self.decoder = LIUMCVC_Decoder(tgt_size,tgt_embedding_size,hidden_size,2*hidden_size,n_layers,dropout_out=dropout_out,attn_type=attn_type,tied_emb=tied_emb)
        
        #Decoder Initialization Layer
        self.decoderini = FF(2*hidden_size,hidden_size,activ='tanh')
        
        #self.decoderini = nn.Linear(2*hidden_size,hidden_size)
        '''
        if bias_zero:
        	torch.nn.init.constant(self.decoderini.bias.data, 0.0)
        '''
    def forward(self,src_var,src_lengths,tgt_var, teacher_force_ratio=1.0,max_length=80,criterion=None):
        ...
        decoder_hidden = self.decoderini(encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1))
        #decoder_hidden = self.decoderini(torch.mean(encoder_outputs, dim=0, keepdim=True))
        ...
        if is_teacher: 
            for di in range(tgt_l):
                ...
                #loss_n = torch.gather(decoder_output,dim=1,index=tgt_var[:,di].unsqueeze(1)).sum()
    ...
    def beamsearch_decode(self,src_var,src_lengths,beam_size=1,max_length=80,tgt_var=None):
        ...
        #Prepare the Input and Output Variables for Decoder
        decoder_input = Variable(torch.LongTensor([[SOS_token] for x in range(batch_size)]))
        decoder_hidden = F.tanh(self.decoderini(encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1)))
