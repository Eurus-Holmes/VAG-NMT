...
class LIUMCVC_Seq2Seq(nn.Module):
    def __init__(self, \
                 src_size, \
                 tgt_size, \
                 src_embedding_size, \
        ...
        super(LIUMCVC_Seq2Seq,self).__init__()
        ...
        self.decoder = LIUMCVC_Decoder(tgt_size,tgt_embedding_size,hidden_size,2*hidden_size,n_layers,dropout_out=dropout_out,attn_type=attn_type,bias_zero=bias_zero)
    ...
    def forward(self,src_var,src_lengths,teacher_force_ratio=0,tgt_var=None,max_length=80,criterion=None):
        ...
        decoder_hidden = F.tanh(self.decoderini(encoder_outputs.sum(0)/context_mask.sum(0).unsqueeze(1))).unsqueeze(0)#Changed by Mingyang
        ...
        if teacher_force_ratio > 0:
            ...
            for b in range(batch_size):
                ...
                outputs[:output_b.size()[0],b,:] = output_b
